...
void CommonUtility::renderNodeToFile(const std::string& fileName, cocos2d::Node* renderNode, const cocos2d::Vec2& offset,const std::function<void(const std::string&)>& callback)
{
    Image::Format format;
    //进行后缀判断
    if(std::string::npos != fileName.find_last_of(".")){
    ...
    //获取屏幕尺寸，初始化一个空的渲染纹理对象
    cocos2d::Size size = renderNode->getContentSize();
    ...
    auto _schedule = Director::getInstance()->getRunningScene()->getScheduler();
    _schedule->schedule(scheduleCallback, Director::getInstance(), 0.0f,0,0.0f, false, "rendernode");
